...
export async function createFollowUpJobs(
  ...
  if (role === 'orchestrator') {
    ...
    const allPassed = testResults?.every((r) => r['status'] === 'passed') ?? false;
    const hasFailures = testResults?.some((r) => r['status'] === 'failed' || r['status'] === 'error') ?? false;

    if (allPassed || (!hasFailures && !testResults?.length)) {
