...
2. 解析 Room + Milestone：
   - 显式："做 user-auth m1" → room=user-auth, milestone=m1
   - "下一个" → 读当前 Room progress.yaml 找下一个 `status: pending`
   - 语义推断："实现 checkout 逻辑" → 匹配 milestone name
   ...
   **Plan 模板（两路径共用）**：
   ...
   Plan {名称}:
     ...
     Corner cases（必覆盖以下类别，缺项要说明为什么不适用）:
       - 空输入: 空数组 / 空字符串 / 空对象
       - 类型边界: 数值 vs 字符串, scalar vs 集合, null/undefined
       - 资源缺失: 依赖未注入, 文件不存在, 字段不存在
       - 名字冲突 / 重复 / shadow
       - 错误传播: 下游 throw 时的处理方式
     优缺点: {至少各 1 条}
   ```
...
10. **Smoke-test real interfaces**（写详细测试前必做）——用 2 秒脚本验证假设：
    ...
    console.log(typeof s.create, typeof s.verify);
    "
    ```
...
15. **汇总 findings → 过滤 → 修复**：
    ...
    🔍 Code Review 汇总
      ...
      必修: ✴️ [C1] Date vs string 类型 mismatch (line 145) → normalize helper
            ✴️ [C2] 空输入未短路 crash 下游 (line 148) → empty guard
            ⚠️ [H1] 边界条件遗漏 (line 264) → 补齐 guard
      LOW（要 defer？）: · [L1] 返回 schema 不统一 → 统一类型定义 / ...
    修复并重跑测试？(y/n/选择性)
    ```
