...
// Bug #3 fix: Detect repo root via .git/ or project.godot instead of package.json
function findRepoRoot(): string {
  let dir = __dirname;
  while (dir !== path.dirname(dir)) {
    // Check for monorepo root (package.json with workspaces) or Godot project marker
...
export function extractKeywords(text: string): string[] {
  ...
  // Expand snake_case:
  //   3. underscores: 'mutation_manager' → 'mutation manager'
  const expanded = text
    .replace(/_/g, ' ')
...
function getTaskTypeNote(taskType: string, role: string): string | null {
  if (role === 'coder') {
    switch (taskType) {
      case 'bug':
        return 'Bug fix: add or update a GUT test that would have caught this bug before marking criteria met.';
      case 'refactor':
        return 'Refactor: all existing GUT tests must pass unchanged. Do not change external behavior — if a test must change, the refactor has drifted into a feature.';
      case 'test':
  ...
  if (role === 'coder' || role === 'tester') {
    // Tester gets the same notes as coder for context
  }
  if (role === 'reviewer') {
    switch (taskType) {
      case 'bug':
        return 'Verify a regression GUT test was added that would have caught this bug.';
      case 'refactor':
        return 'Verify no existing GUT test had to change behavior to accommodate the refactor.';
      case 'feature':
        return 'Verify the PR includes GUT tests covering the new behavior. Check resource references use AssetManager (no bare load()).';
      case 'test':
        return 'Verify the new GUT tests run in isolation. Tests must extends GutTest.';
      default:
        return null;
    }
  }
