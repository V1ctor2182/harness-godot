...
export function validatePlan(plan: Plan): string[] {
  ...
  // If two parallel tasks modify the same .tscn or global data file without blockedBy, reject
...
/**
 * Bug #5: Validate that parallel tasks don't modify the same .tscn or data/global/ files
 * without a blockedBy dependency between them.
 */
function validateFileMutex(tasks: Array<Record<string, unknown>>): string[] {
  ...
  for (let i = 0; i < tasks.length; i++) {
    ...
    const mutexFilesI = filesI.filter((f) =>
      MUTEX_PATTERNS.some((p) => f.endsWith(p) || f.includes(p))
    );
    ...
    for (let j = i + 1; j < tasks.length; j++) {
      ...
      // Check if there's a blockedBy dependency in either direction
      const blockedByI = (tasks[i]['blockedBy'] as number[] | undefined) ?? [];
      const blockedByJ = (tasks[j]['blockedBy'] as number[] | undefined) ?? [];
      const hasDep = blockedByI.includes(j) || blockedByJ.includes(i);

      if (!hasDep) {
        errors.push(
          `Tasks ${i} and ${j} both modify ${shared.join(', ')} but have no blockedBy dependency. ` +
            `Add blockedBy to serialize access to .tscn and data/global/ files.`
        );
      }
    }
  }
