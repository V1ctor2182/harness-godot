...
func test_horizontal_movement_speed() -> void:
	...
	assert_almost_eq(vel.length(), Player.SPEED, 0.01, "horizontal movement magnitude should equal SPEED (200.0)")
...
func test_diagonal_movement_is_normalized() -> void:
	...
	assert_almost_eq(vel.length(), Player.SPEED, 0.01, "diagonal movement must be normalized to SPEED (200.0), not SPEED * sqrt(2)")
